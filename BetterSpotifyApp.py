...
from layout_factories.app_utils.app_utils import to_timer, to_timer_with_hours
import pprint
...
from layout_factories.context_table_layouts import set_playlist_details, get_playlist_tracks_table
from layout_factories.rmb_menu_layouts import Rmb_on_track_menu
...
class BetterSpotifyApp(MDApp):
    ...
    def __init__(self, **kwargs):
        ...
        self.rmb_track_menu = Rmb_on_track_menu()
        if self.spotify.devices()['devices']:
    ...
    def selected_playlist(self, playlist):
        if not 'snapshot_id' in playlist.keys():
    ...
    def get_progress_slider(self):
        if self.progress_in_mins:
            minutes = self.progress_in_mins[0]
            seconds = self.progress_in_mins[1]
        else:
            minutes = 0
            seconds = 0
        if self.remaining_in_mins:
            remaining_minutes = self.remaining_in_mins[0]
            remaining_seconds = self.remaining_in_mins[1]
        else:
            remaining_minutes = 0
            remaining_seconds = 0

        if not self.left_timer:
            self.left_timer = MDLabel(text=f'[size=11]{minutes}:{seconds}[/size]',
                                      size_hint=(.07, 1),
                                      pos_hint={"center_x": 0,
                                                "center_y": .12},
                                      markup=True,
        ...
        if not self.right_timer:
            self.right_timer = MDLabel(text=f'[size=11]{remaining_minutes}:{remaining_seconds}[/size]',
                                       size_hint=(.07, 1),
                                       pos_hint={"center_x": 1,
                                                 "center_y": .12},
                                       markup=True,
        ...
        self.left_timer.text = f'[size=11]{minutes}:{seconds}[/size]'
        self.right_timer.text = f'[size=11]{remaining_minutes}:{remaining_seconds}[/size]'
        try:
            val = self.current_track['progress_ms']
        except TypeError:
            val = 0
    ...
    def open_right_clicked_track_menu(self, caller=None, touch=None):
        self.rmb_track_menu.set_rmb_menu_items()
        self.rmb_track_menu.set_params(caller, touch)
    ...
    def changed_context_to_playlist(self, playlist_id):
        ...
        self.tracks_table = []
        playlist_details_widget = set_playlist_details(self.selected_playlist)
        self.screen.ids.main_screen.clear_widgets()
        self.screen.ids.main_screen.add_widget(playlist_details_widget)
    ...
    def get_tracks(self, iter):
        ...
        self.selected_playlists_tables[self.selected_playlist['id']
                                       ] = self.selected_playlist
        self.selected_playlists_widgets[self.selected_playlist['id']
                                        ] = self.screen.ids.main_screen.children[::-1]
